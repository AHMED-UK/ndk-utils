...
# Robust download utility to be used ONCE per project
download_source() {
    local url=$1
...
# --- PRE-DOWNLOAD SECTION (Run once to avoid Google 503/Throttling) ---
...
curl -L https://github.com/mirror/ncurses/archive/refs/tags/v6.5.tar.gz -o "$SRC_CACHE/ncurses.tar.gz"
...
for ABI in "${ABIS[@]}"; do
    ...
    # 5. LZMA (Manual build using AOSP list)
    ...
    for src in "${LZMA_SRCS[@]}"; do
        $CC $CFLAGS $LZMA_FLAGS -IC/ -c "$src" -o "$(basename ${src%.c}.o)"
    done
